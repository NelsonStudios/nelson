...
You can easily check this in the **app/code/Serfe/ExternalCart/Api/CartInterface.php** file.
  ...
  * **POST: addToCart():** Function to add products into cart similar to addProductIntoCart(), but you can add products without generate token first (since it's autogenerated) and if already exist a token in session, that will be used. For more information take a look at the below examples.
